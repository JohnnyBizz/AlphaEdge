// ── AI Signal Engine (profile-aware) ─────────────────────
// Generates technical analysis via the Anthropic API, adapted
// to each user's trader profile from onboarding.
...
// ── Profile → plain-english description ──────────────────
...
function describeProfile(p: TraderProfile): string {
  ...
  const riskMap = {
    conservative: 'conservative risk management: tight stop losses of 2–3%, only the cleanest setups with 80%+ confidence',
    balanced:     'balanced risk management: standard stop losses of 4–5%, mix of safe and opportunistic analysis',
    aggressive:   'aggressive risk management: wider stop losses of 6–10%, comfortable with higher-risk/higher-reward setups',
  }
  return `
TRADER PROFILE:
...
${p.trade_style === 'position' ? '- Favor weekly trends, fundamental catalysts, and macro support levels.' : ''}
${p.profit_target === 'quick'    ? '- Set conservative targets close to current price. Prioritize high confidence (80%+) analysis only.' : ''}
${p.profit_target === 'home_run' ? '- Set ambitious targets using Fibonacci extensions. Confidence as low as 60% is acceptable.' : ''}
${p.risk_tolerance === 'conservative' ? '- Only generate BUY analysis with 3+ confirming indicators. Stop loss must be within 3% of entry.' : ''}
...
const BASE_SYSTEM_PROMPT = `You are a professional quantitative analyst producing educational technical analysis.

You analyze market data including technical indicators (RSI, MACD, Bollinger Bands, VWAP, volume) to generate structured educational analysis. Your output is informational only — always frame reasoning as analysis of what the data shows, never as personalized advice.

For each asset you receive, return a JSON object with this exact structure:
{
  "signal_type": "buy" | "sell" | "watch",
  ...
  "ai_reasoning": "<2-3 sentence professional analysis explaining what the indicators show and what to watch for. Frame as 'the data suggests' / 'indicators show', never 'you should'>"
}
...
export async function generateSignalForAsset(
  ...
  const prompt = `Analyze this ${snapshot.market} asset and generate the structured analysis JSON.${profileSection}
...
// ── Helper: fetch a user's trader profile ─────────────────
